  ...
  mysqlconn.selectData(function (error, results) {
    if (error) {
      ...
      if (results.length === 0) {
        ...
        const mysql_insert_query = "insert into " + PROJECT_TABLE + " (title,ownerUserId,description,budgetLow,budgetHigh,skills) values('" + req.body.title + "'," + userId + ",'" + req.body.description + "'," + req.body.budgetLow + "," + req.body.budgetHigh + ",'" + skills + "')";
...
router.post('/projectandbids', function (req, res) {
  ...
  const mysql_select_query = "select u.userId, u.name, u.username, u.contact, p.projectId, p.title, p.description, " + "p.budgetLow, p.budgetHigh, p.skills, p.status From " + USER_TABLE + " as u INNER JOIN " + PROJECT_TABLE + " as p on u.userId = p.ownerUserId AND p.projectId = '" + req.body.projectId + "'";
      ...
      if (results.length === 0) {
        ...
        mysqlconn.selectData(function (error, results) {
          if (error) {
            ...
            const select_inner_join_query = "select u.userId, u.name, u.username, u.contact, u.aboutMe, " + "u.skills, pb.projectId, pb.bidAmount, pb.periodInDays From " + USER_TABLE + " as u INNER JOIN " + PROJECT_BID_TABLE + " as pb on u.userId = pb.userId AND pb.projectId=" + req.body.projectId + " AND u.userId IN (" + ids.join(',') + ")";
