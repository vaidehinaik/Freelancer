...
router.post('/userinfo', function(req, res, next) {
  ...
  const mysql_select_query = "select userId, name, username, contact, aboutMe, skills, photoUrl from "
                              + USER_TABLE
    ...
    if (error) {
      ...
      throw error;
        ...
        if (results.length == 0) {
            ...
            const mysql_update_query = "UPDATE " + USER_TABLE + " SET name = '" + name + "', "
                                      + "contact = '" + contact + "', " + "skills = '" + skills + "', "
                                      + "aboutMe = '" + aboutMe + "' where userId = " + userId;
