    ...
    if (error) {
      ...
      throw error;
        ...
        if (results.length === 0) {
            ...
            const mysql_select_query_2 = "select * from " + PROJECT_BID_TABLE
                                        + " where userId = " + userId + " AND projectId = " + req.body.projectId;
            mysqlconn.selectData(function(error, results) {
              if (error) {
                ...
                if(results.length === 0) {
                  final_query = "Insert INTO " + PROJECT_BID_TABLE + " (projectId, userId, bidAmount, periodInDays) values("
                                + req.body.projectId + "," + userId + "," + req.body.bidAmount + "," + req.body.periodInDays + ")";
                } else {
                  final_query = "Update " + PROJECT_BID_TABLE + " SET projectId = " + req.body.projectId
                                + ", userId = " + userId + ", bidAmount = " + req.body.bidAmount
                                + ", periodInDays = " + req.body.periodInDays
