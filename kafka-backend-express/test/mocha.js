...
describe('Signup', function() {
  ...
  describe('/users/signup', function() {
    it('responds with status 201', function(done) {
      chai.request(app)
        .post('/users/signup')
        .send({name:"mocha" + num, username:"mocha_user" + num, password:"mocha"})
...
describe('Login', function() {
  describe('/users/login', function() {
    it('responds with status 201', function(done) {
      chai.request(app)
        .post('/users/login')
        .send({username:"vaidehin", password:"vaidehi"})
        .end(function(err, res) {
          should.not.exist(err);
          res.status.should.equal(201);
          res.type.should.equal('application/json');
...
describe('Login Failure', function() {
  describe('/users/login', function() {
    it('responds with status 401', function(done) {
      chai.request(app)
        .post('/users/login')
        .send({username:"xyz", password:"wronguser"})
        .end(function(err, res) {
